...
public final class RatTerm {
  ...
  public boolean isNaN() {
    if (this.coeff == RatNum.NaN) return true;
    return false;
  }
  /*public void main(String[] args) {
	  RatTerm q = new RatTerm(RatNum.NaN,0);
	  System.out.println(q.isZero());
  }*/
  /**
  ...
  public boolean isZero() {	
    if (this.coeff != RatNum.NaN && this.coeff == RatNum.ZERO) return true;
    return false;
  }
  ...
  public double eval(double d) {
	  ...
	  double powerresult = Math.pow(d, expt);
	  return coeff.doubleValue() * powerresult;
  }
  ...
  public RatTerm negate() {
    if (this.isNaN() == true) return RatTerm.NaN;
  ...
  public RatTerm add(RatTerm arg) {
    ...
    if(this.isZero() == true) return new RatTerm(this.coeff.add(arg.coeff), arg.expt);
	if(arg.isZero() == true) return new RatTerm(this.coeff.add(arg.coeff), this.expt);
  ...
  public RatTerm sub(RatTerm arg) {
	  checkRep();
	  if (this.expt != arg.expt && this.isNaN() == false && arg.isNaN() == false && this.isZero() == false && arg.isZero() == false) {
	    	throw new IllegalArgumentException("not subtractable");
	    }
	  if (this.isNaN() == true || arg.isNaN() == true) {
	    	return RatTerm.NaN;
	    }
	  if(this.isZero() == true) return new RatTerm(this.coeff.sub(arg.coeff), arg.expt);
	  if(arg.isZero() == true)return new RatTerm(this.coeff.sub(arg.coeff), this.expt);
  ...
  public RatTerm mul(RatTerm arg) {
	  ...
	  int newexpt = arg.expt + this.expt;
	  return new RatTerm(arg.coeff.mul(this.coeff), newexpt);
  }
  ...
  public RatTerm div(RatTerm arg) {
	  ...
	  if (arg.isZero()) return RatTerm.NaN;
	  int newexpt = this.expt - arg.expt;
	  return new RatTerm(this.coeff.div(arg.coeff), newexpt);
  }
  ...
  public RatTerm differentiate() {
	  checkRep();
	  RatTerm q = new RatTerm(RatNum.NaN,0);
      if (this.isNaN() == true) return q;
      if (this.expt == 0) return RatTerm.ZERO; 
      RatNum expRat = new RatNum(this.expt,1);  
      RatNum newcoeff = coeff.mul(expRat);
	  return new RatTerm(newcoeff,this.expt - 1);  
  }
  ...
  public RatTerm antiDifferentiate() {
	  checkRep();
      if (this.isNaN() == true) return RatTerm.NaN;
      RatNum expRat = new RatNum(this.expt + 1,1);
	  RatNum newcoeff = coeff.div(expRat);
	  return new RatTerm(newcoeff,this.expt + 1);
  }
